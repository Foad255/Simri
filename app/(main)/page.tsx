'use client'
import {
  ArrowRight, BarChart3, Brain, Database, Dna, FileUp,
  Lightbulb, Search, UploadCloud, Users, Zap
} from 'lucide-react';
...
// WORKFLOW STEP
const WorkflowStep = ({ number, title, description, icon: Icon }: { number: string, title: string, description: string, icon: React.ElementType }) => (
...
// GLOBAL STYLES (unchanged from yours)
const GlobalStyles = () => (
  <style jsx global>{`[INSERT YOUR CSS KEYFRAMES HERE]`}</style>
);
    ...
    <>
      ...
      <div className="bg-slate-100 min-h-screen font-sans text-slate-800">
        ...
        <section className="relative bg-gradient-to-br from-blue-700 via-sky-700 to-teal-600 py-32 md:py-44 text-white border-b border-teal-800 overflow-hidden">
          <div className="container mx-auto px-6 text-center relative z-10">
            ...
            <div className="flex flex-col sm:flex-row justify-center items-center space-y-5 sm:space-y-0 sm:space-x-6">
              ...
              <Link href="#how-it-works" className="bg-transparent text-white border-2 border-sky-400/70 px-10 py-4 rounded-lg font-semibold text-lg hover:bg-white hover:text-teal-600 transition-all duration-300 transform hover:scale-105 w-full sm:w-auto group flex items-center">
        ...
        <section className="py-24 md:py-32 bg-white">
          <div className="container mx-auto px-6">
            <div className="text-center mb-20 md:mb-24">
              ...
              <p className="text-slate-600 max-w-2xl mx-auto text-lg">Medical imaging researchers face a growing flood of data — Simri helps you explore it intelligently.</p>
            </div>
        ...
        <section id="how-it-works" className="py-24 md:py-32 bg-slate-50 border-t border-b border-slate-200/80">
          <div className="container mx-auto px-6">
            ...
            <p className="text-center text-slate-600 max-w-3xl mx-auto mb-20 text-lg">
              From raw MRIs to real patient insight in four steps.
            </p>
            <div className="max-w-3xl mx-auto relative">
              ...
              <div className="space-y-12 md:space-y-14 relative z-10">
                <WorkflowStep number="1" title="Upload Scans" description="Upload T1c, T1n, T2f, T2w, and segmentation masks via secure AWS S3." icon={UploadCloud} />
                <WorkflowStep number="2" title="Embedding Generation" description="The TIEP model generates a deep embedding vector representing the scan." icon={Brain} />
                <WorkflowStep number="3" title="Vector Similarity Search" description="We search across the BraTS2025 dataset using MongoDB’s fast vector index." icon={Search} />
                <WorkflowStep number="4" title="Compare in 3D Viewer" description="Matched cases are shown next to your scan in our Niivue-based 3D viewer." icon={BarChart3} />
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-24 md:py-32 bg-white">
          <div className="container mx-auto px-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 md:gap-12">
              <FeatureCard icon={Dna} title="Multi-Sequence MRI Support" description="Handles common sequences: T1c, T1n, T2f, T2w, and segmentation masks." />
              <FeatureCard icon={Brain} title="TIEP Deep Learning Model" description="Custom-trained to generate meaningful scan embeddings for comparison." />
              <FeatureCard icon={Search} title="MongoDB Vector Search" description="Fast search at scale — thousands of cases matched in milliseconds." />
              <FeatureCard icon={Zap} title="3D MRI Viewer" description="Explore results side-by-side using Niivue in-browser visualization." />
              <FeatureCard icon={Database} title="Scalable Infrastructure" description="Built on AWS S3, Flask, MongoDB Atlas, and Next.js for reliability." />
              <FeatureCard icon={Users} title="Cohort Discovery" description="Easily surface groups of patients with similar imaging patterns." />
            </div>
          </div>
        </section>
        ...
        {/* <section className="bg-slate-100 py-24 text-center">
          <h3 className="text-3xl font-bold mb-4">Want Early Access or Have Feedback?</h3>
          <p className="text-slate-600 mb-6 max-w-xl mx-auto">
            We&apos;re working with researchers and radiologists to shape Simri. Get notified or tell us how you&apos;d use it.
          </p>
          <form className="max-w-md mx-auto flex flex-col sm:flex-row items-center gap-4">
            <input type="email" placeholder="Your email" className="px-5 py-3 w-full rounded-lg border border-slate-300 shadow-sm focus:ring-2 focus:ring-teal-400" />
            <button type="submit" className="bg-gradient-to-r from-sky-500 to-emerald-500 text-white px-6 py-3 rounded-lg font-semibold hover:shadow-lg transition-all duration-300">
              Get Notified
            </button>
          </form>
        </section> */}
